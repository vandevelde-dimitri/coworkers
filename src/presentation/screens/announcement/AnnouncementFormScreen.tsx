...
} from "@/src/domain/entities/announcement/Announcement";
...
export default function AnnouncementFormScreen({
  ...
  useEffect(() => {
    if (isEditMode && announcement) {
      reset({
        ...
        dateEnd: announcement.dateEnd ? new Date(announcement.dateEnd) : null,
      });
    } else if (!isEditMode) {
      reset({
        title: "",
  ...
  const onSubmit = (
    data: CreateAnnouncementPayload | UpdateAnnouncementPayload,
  ) => {
    if (isEditMode) {
      updateAnnouncement({
        id: announcementId!,
        payload: data as UpdateAnnouncementPayload,
      });
      return;
    }
    createAnnouncement(data as CreateAnnouncementPayload);
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <AppButton
            disabled={
              pendingCreate || pendingUpdate || (isEditMode && !isDirty)
            }
